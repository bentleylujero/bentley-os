...
// Real OpenCode build tasks (multi-step tool use) can run well past the
// undici default 5-minute headers/body timeout. Raise it for this whole
// process — this is the service that talks to OpenCode.
...
app.post('/execute', async (c) => {
  ...
  try {
    ...
    await audit({ action: 'contractor.execute', outcome: 'error', payload: { spec, error: message, cause } });
    return c.json({ error: 'execute failed', detail: message, cause }, 502);
  }
});
const port = 4100;
